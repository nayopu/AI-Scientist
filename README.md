...
We're excited to introduce **The AI Scientist**, a comprehensive system for fully automatic scientific discovery and creative design, enabling Foundation Models such as Large Language Models (LLMs) to perform research and design tasks independently. The framework now supports both traditional scientific research and innovative game design, including social deduction games.
...
**The AI Scientist** also generates comprehensive game manuals for social deduction games 🎮, including innovative mechanics, detailed role descriptions, and complete gameplay instructions. The game design template creates fully playable games with unique themes and balanced mechanics.
...
3. [Setting Up the Templates](#setting-up-the-templates)
   ...
   - [Social Deduction Game Template](#social-deduction-game-template)
4. [Run AI Scientist Experiments](#run-ai-scientist-experiments)
5. [Getting an LLM-Generated Review](#getting-an-llm-generated-review)
...
We provide multiple templates covering both scientific research and creative design domains. The scientific templates include: **NanoGPT**, **2D Diffusion**, and **Grokking**. Additionally, we now provide a **Social Deduction Game** template for automated game design. These templates enable The AI Scientist to generate ideas and conduct experiments or create designs in these areas. We accept contributions of new templates from the community, but please note that they are not maintained by us. All templates beyond the four core ones are community contributions.
...
#### OpenRouter API (Llama3.1, Perplexity Search)

By default, this uses the `OPENROUTER_API_KEY` environment variable. OpenRouter also provides access to Perplexity's search capabilities for game design novelty checking. Set your API key:
...
### Social Deduction Game Template
...
python launch_scientist.py --experiment social_deduction_game --max-ideas 2 \
  --player-model "openrouter:anthropic/claude-3-5-sonnet" \
...
- `--search-api perplexity` (requires OPENROUTER_API_KEY)
- `--search-api openai` (requires OPENAI_API_KEY)

If you have more than one GPU, use the `--parallel` option to parallelize ideas across multiple GPUs.

## Getting an LLM-Generated Review

The system can review both research papers and game manuals:
...
```python
import openai
from ai_scientist.perform_review import load_paper, perform_game_manual_review

client = openai.OpenAI()
model = "gpt-4o-2024-05-13"

# Load game manual from PDF file (raw text)
manual_txt = load_paper("game_manual.pdf")

# Get the review dictionary focused on game design criteria
...
# Inspect review results
review["Overall"]      # Overall score (1-10)
review["Completeness"] # Completeness score
review["Clarity"]      # Clarity score  
review["Conciseness"]  # Conciseness score
```
...
If there is an area of study or creative domain you would like **The AI Scientist** to explore, it is straightforward to create your own templates. In general, follow the structure of the existing templates, which consist of:
...
- `prompt.json` — Put information about your template here, including system prompts and task descriptions.
- `seed_ideas.json` — Place example ideas here. For research templates, these are experimental ideas. For game design templates, these are game concepts. You can also try to generate ideas without any examples and then pick the best one or two to put here.
- `latex/template.tex` — We recommend using our LaTeX folder but be sure to replace the pre-loaded citations with ones that you expect to be more relevant. For game design templates, this creates game manuals instead of research papers.
...
- Social Deduction Game Design (`social_deduction_game`) - Enhanced framework for automated social deduction game creation with web-based novelty checking

*This section is reserved for community contributions. Please submit a pull request to add your template to the list! Please describe the template in the PR description, and also show examples of the generated papers or game manuals.*

## Template Resources

We provide four core templates, which use code from various repositories, credited below:
...
- **Social Deduction Game Template** is a custom framework designed for automated game design, featuring web-based novelty checking via multiple search APIs (DuckDuckGo, Perplexity, OpenAI).
...
Change the base `template.tex` files contained within each template. For game design templates, this controls the game manual format instead of research paper format.

**How do I run The AI Scientist for different subject fields?**

Please refer to the instructions for different templates. In this current iteration, this is restricted to ideas that can be expressed in code. The framework now supports both scientific research and creative design domains like game design. However, lifting this restriction would represent exciting future work! :)
